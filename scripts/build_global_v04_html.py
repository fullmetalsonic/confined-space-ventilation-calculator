...
SOURCE = ROOT / "밀폐공간_환기량_산정_도구.html"
OUTPUT = ROOT / "dist" / "html" / "밀폐공간_환기량_산정_도구_v0.4.html"
...
html = html.replace("v0.3", "v0.4")
for old, new in (
    ("한국어 결과서는 항상 먼저 출력됩니다. 선택한 언어마다 새 페이지가 추가됩니다. 번역문은 이해 보조용이며 한국어 원문과 회사 표준절차가 우선합니다.",
     "현재 화면 언어가 기본 결과서로 먼저 출력됩니다. 선택한 추가 언어마다 새 페이지가 추가됩니다."),
    ("한국어 결과서는 항상 먼저 출력됩니다. 선택한 언어마다 새 페이지가 추가되며 한국어 원문과 회사 표준절차가 우선합니다.",
     "현재 화면 언어가 기본 결과서로 출력되며 선택한 추가 언어마다 새 페이지가 추가됩니다."),
    ("The Korean report prints first. Each selected language adds a page; the Korean original and company procedure prevail.",
     "The current UI language is the primary report; each selected supplementary language adds a page."),
    ('"choiceB_title":"已知发电率"', '"choiceB_title":"已知污染物产生率"'),
    ('"choiceB_title":"已知發電率"', '"choiceB_title":"已知污染物產生率"'),
    ('"resultA":"交易量计算结果"', '"resultA":"换气量计算结果"'),
    ('"resultA":"交易量計算結果"', '"resultA":"換氣量計算結果"'),
    ("10 个空间卷用于初始清除", "10 倍空间体积用于初始吹扫"),
    ("10 個空間磁碟區用於初始清除", "10 倍空間體積用於初始吹掃"),
    ('"optionalDuct":"Opsiyonel kanal va statik bosim sharoitlari"',
     '"optionalDuct":"Ixtiyoriy kanal va statik bosim sharoitlari"'),
):
    html = html.replace(old, new)

html = replace_once(
    html,
    "const methods=printText.methods || UI_DETAIL_EN.fanMethods;",
    "const methods=V04_FAN_METHODS[currentUiLanguage] || printText.methods || UI_DETAIL_EN.fanMethods;",
    "flow-method UI language binding",
)

html = replace_once(
    html,
    "</head>\n<body>",
    """<style id="global-v04-design">
  html{scroll-padding-top:12px;}
  body{font-family:"Noto Sans","Noto Sans KR","Noto Sans Arabic","Noto Sans Devanagari",
    "Noto Sans Bengali","Noto Sans Thai","Noto Sans Khmer","Noto Sans Myanmar",
    "Segoe UI","Malgun Gothic",Arial,sans-serif;}
  .skip-link{position:fixed;left:12px;top:-80px;z-index:9999;padding:10px 14px;border-radius:8px;
    background:#fff;color:#075985;box-shadow:0 8px 24px #002b4933;font-weight:800;}
  .skip-link:focus{top:12px;}
  /* Progressive fallback: JavaScript goStep() overrides these inline. */
  .step{display:none;}
  .step[data-step="1"]{display:block;}
  .global-profile{display:grid;grid-template-columns:minmax(220px,1.25fr) minmax(190px,1fr) 145px 170px;
    gap:12px;margin:0 0 14px;padding:15px;border:1px solid #bed8e6;border-radius:15px;
    background:linear-gradient(135deg,#f8fcff,#eef8fc);box-shadow:0 6px 20px #0759850d;}
  .global-profile .field{margin:0;min-width:0;}
  .global-profile .profile-note{grid-column:1/-1;margin:0;padding:10px 12px;border-left:4px solid #1686b4;
    border-radius:7px;background:#fff;color:#345565;font-size:12px;line-height:1.55;}
  .global-profile .profile-source{grid-column:1/-1;display:flex;gap:8px;flex-wrap:wrap;align-items:center;
    font-size:11px;color:#56717e;}
  .global-profile .profile-source a{color:#066f9e;font-weight:750;}
  .validation-summary{display:none;margin:0 0 14px;padding:12px 14px;border:1px solid #e7a49e;
    border-left:5px solid #c0392b;border-radius:10px;background:#fff4f2;color:#792b25;font-weight:700;}
  .validation-summary.show{display:block;}
  input[type="text"],textarea{unicode-bidi:plaintext;}
  .choice[role="radio"]:focus-visible,.stepper li[role="button"]:focus-visible{
    outline:3px solid #ffbf47;outline-offset:3px;}
  .global-print-meta{display:grid;grid-template-columns:1fr 1fr;gap:5px;margin:5px 0 7px;padding:6px 8px;
    border:1px solid #b9d4e1;border-radius:7px;background:#f7fbfd;font-size:8px;}
  .global-print-meta span{color:#617680;font-weight:700;}
  .global-print-meta b{color:#173f52;overflow-wrap:anywhere;}
  .global-print-meta .global-profile-guidance{grid-column:1/-1;margin:2px 0 0;padding-top:5px;
    border-top:1px solid #d5e5ec;color:#425e6c;line-height:1.4;}
  .report table th,.report table td,.translated-report table th,.translated-report table td{
    white-space:normal;overflow-wrap:anywhere;word-break:normal;hyphens:auto;}
  .report-source-table td:first-child,.report-source-table td:first-child b{
    max-width:100%;white-space:normal;overflow-wrap:anywhere;word-break:break-word;}
  .translated-legal-table{margin:4px 0 6px;font-size:8.4px;}
  .translated-legal-table td:first-child{width:28%;font-weight:800;color:#075985;}
  .translated-legal-source{font-size:7.6px;line-height:1.35;color:#516a76;}
  .unit-mode-note{display:block;margin-top:4px;color:#52707e;font-size:10px;line-height:1.35;}
  #report-card[data-primary-lang]:not([data-primary-lang="ko"]) > #print-title,
  #report-card[data-primary-lang]:not([data-primary-lang="ko"]) > #report-body{display:none;}
  .translated-report.primary-translated-report{
    page-break-before:auto!important;break-before:auto!important;border-top:0;}
  /* Korean supplementary output is a native report, not a translated-card report. */
  .korean-supplement-report{margin:28px 0 0;padding:16px 0 0;border-top:3px double var(--navy);}
  .korean-supplement-report > .print-title{display:flex;}
  @media(max-width:720px){
    .global-profile{grid-template-columns:1fr;}
    .global-profile .profile-note,.global-profile .profile-source{grid-column:1;}
    .report-source-table col:first-child{width:30%!important;}
    .report-source-table col:last-child{width:70%!important;}
    .report-source-table td:first-child{padding-left:7px;padding-right:7px;font-size:11.5px;line-height:1.35;}
    .report-source-table td:nth-child(2){font-size:11.5px;line-height:1.45;}
  }
  @media print{
    .global-profile,.validation-summary,.skip-link{display:none!important;}
    .global-print-meta{break-inside:avoid;page-break-inside:avoid;}
    .translated-report.primary-translated-report{
      page-break-before:auto!important;break-before:auto!important;}
    .korean-supplement-report{
      page-break-before:always!important;break-before:page!important;
      margin:0!important;padding:0!important;border:0!important;}
    /* Supplementary pages share a clean text-first overview.  The native Korean
       report normally uses boxed overview cards, which look inconsistent next
       to the other supplementary-language pages. */
    .korean-supplement-report .print-overview-grid{gap:1px 5px;}
    .korean-supplement-report .print-overview-item{
      min-height:0;padding:2px 1px;border:0;border-radius:0;
      border-bottom:1px solid #D5E1E7;background:transparent;}
    .korean-supplement-report .print-overview-item span{margin-bottom:0;}
  }
</style>
</head>
<body>
<a class="skip-link" href="#main-content">Skip to main content</a>""",
    "global style and skip link",
)

html = replace_once(
    html,
    '<div class="shell">\n\n  <div class="session-bar no-print">',
    """<div class="shell" id="main-content" tabindex="-1">

  <section class="global-profile no-print" aria-labelledby="global-profile-title">
    <div class="field">
      <label id="global-profile-title" for="jurisdiction-profile">🌍 <span id="profile-label">안전 기준 프로필</span></label>
      <select id="jurisdiction-profile" onchange="setJurisdictionProfile(this.value)" aria-describedby="profile-note">
        <option value="kr">대한민국 · KOSHA H-80-2021</option>
        <option value="us-general">United States · OSHA 29 CFR 1910.146</option>
        <option value="us-construction">United States · OSHA 29 CFR 1926 Subpart AA</option>
        <option value="uk">United Kingdom · Confined Spaces Regulations 1997 / HSE L101</option>
        <option value="au">Australia · WHS Regulations / Model Code of Practice</option>
        <option value="sg">Singapore · WSH (Confined Spaces) Regulations 2009</option>
      </select>
    </div>
    <div class="field">
      <label for="profile-reference"><span id="profile-reference-label">추가 회사 기준 문서·개정번호</span> <span class="opt">(선택)</span></label>
      <input id="profile-reference" type="text" maxlength="160" dir="auto" placeholder="예: HSE-CS-01 Rev.4">
    </div>
    <div class="field">
      <label for="paper-size"><span id="paper-label">인쇄 용지</span></label>
      <select id="paper-size" onchange="setPrintPaper(this.value)">
        <option value="A4">A4</option>
        <option value="Letter">US Letter</option>
      </select>
      <small class="hint" id="paper-support-note"></small>
    </div>
    <div class="field">
      <label for="unit-system"><span id="unit-label">표시 단위</span></label>
      <select id="unit-system" onchange="setV04UnitSystem(this.value,false)">
        <option value="si">SI · m³ / m³/h</option>
        <option value="us">US · ft³ / CFM + SI</option>
      </select>
      <small class="unit-mode-note" id="unit-support-note"></small>
    </div>
    <p class="profile-note" id="profile-note" role="status"></p>
...
html = replace_once(
    ...
    '  <div class="validation-summary no-print" id="validation-summary" role="alert" aria-live="assertive"></div>\n\n  <ul class="stepper" id="stepper" aria-label="Workflow steps"></ul>',
    "validation summary",
)

html = html.replace(
    '<div class="choice" data-mode="A" onclick="selectMode(\'A\')">',
    '<div class="choice" data-mode="A" role="radio" aria-checked="false" tabindex="0" onclick="selectMode(\'A\')">',
)
html = html.replace(
    '<div class="choice" data-mode="B" onclick="selectMode(\'B\')">',
    '<div class="choice" data-mode="B" role="radio" aria-checked="false" tabindex="0" onclick="selectMode(\'B\')">',
)
html = html.replace(
    '<div class="choice" data-mode="C" onclick="selectMode(\'C\')">',
    '<div class="choice" data-mode="C" role="radio" aria-checked="false" tabindex="0" onclick="selectMode(\'C\')">',
)

# Escape user-controlled strings before inserting them into HTML attributes/cells.
html = html.replace('value="${z.name||\'\'}"', 'value="${escapeV04(z.name||\'\')}"')
html = html.replace('value="${f.name}"', 'value="${escapeV04(f.name)}"')
html = html.replace('value="${f.advancedNote||\'\'}"', 'value="${escapeV04(f.advancedNote||\'\')}"')
html = html.replace('${f.name||\'-\'}${advanced?', '${escapeV04(f.name||\'-\')}${advanced?')
html = html.replace(
...
html = replace_once(
    ...
    "step navigation scroll target",
)
for old, new in (
    ("${requiredQ.toFixed(1)}", "${formatV04Number(requiredQ,1)}"),
    ("${requiredQ.toFixed(2)}", "${formatV04Number(requiredQ,2)}"),
    ("${totalSupply.toFixed(1)}", "${formatV04Number(totalSupply,1)}"),
    ("${totalSupply.toFixed(2)}", "${formatV04Number(totalSupply,2)}"),
    ("${state.volume.toFixed(2)}", "${formatV04Number(state.volume,2)}"),
    ("${(r.requiredQ||0).toFixed(2)}", "${formatV04Number(r.requiredQ||0,2)}"),
    ("${(r.G||0).toFixed(4)}", "${formatV04Number(r.G||0,4)}"),
):
    html = html.replace(old, new)

html = replace_once(
    html,
    "  sections.push(buildPermitChecklistHTML());",
    "  sections.push(buildV04PermitChecklistHTML(r.mode));",
    "profile checklist",
)
html = replace_once(
    html,
    """  sections.push(`<div class="note danger"><b>출입허가 판단 주의:</b> 본 결과는 환기계획을 위한 참고값이며 출입허가서나 농도 측정기록을 대체하지 않습니다. 작업 시작·재개 전에는 제619조의2에 따른 측정·평가를 실시하고, 제620조에 따라 작업 전·중 적정공기가 유지되도록 환기해야 합니다. 부적정·경보·이상징후가 있으면 즉시 작업을 중지하고 대피하십시오.</div>`);""",
    "  sections.push(buildV04SafetyNotice());",
    "profile safety notice",
)
html = replace_once(
    html,
    "  sections.push(buildReferenceHTML(r.mode));",
    "  sections.push(buildV04ReferenceHTML(r.mode));",
    "profile references",
)
html = html.replace(
    "<span>적용 환기기준</span><b>${criterionValue}</b>",
    "<span>적용 환기기준</span><b>${formatV04Criterion(criterionValue)}</b>",
)
html = html.replace(
    "const saved = localStorage.getItem('ventcalc-ui-language');",
    "const saved = safeV04StorageGet('ventcalc-ui-language');",
)
html = html.replace(
    "localStorage.setItem('ventcalc-ui-language',code);",
    "safeV04StorageSet('ventcalc-ui-language',code);",
)

extension = r"""
/* ============================================================
   v0.4 GLOBAL SAFETY EXTENSION
   UI language and jurisdiction are intentionally independent.
============================================================ */
const V04_PROFILES = {
  kr:{
    label:'대한민국 · KOSHA H-80-2021',
    noteKo:'대한민국 사전검토 프로필입니다. 현재 체적 10배·작업 중 20회/h 기본값을 사용하되, 최신 법령·KOSHA GUIDE와 사업장 작업허가 절차를 반드시 대조하십시오.',
    noteEn:'Korea planning profile. The current defaults are 10 space volumes for initial purge and 20 ACH during work. Verify current law, KOSHA guidance and company procedures.',
    source:'산업안전보건기준에 관한 규칙 제618~625조 / KOSHA GUIDE H-80-2021',
    url:'https://law.go.kr/LSW/lsLinkCommonInfo.do?chrClsCd=010202&lsJoLnkSeq=1025460863',
    extraSources:[['KOSHA GUIDE 검색','https://smartsearch.kosha.or.kr/']]
  },
  'us-general':{
    label:'United States · OSHA 29 CFR 1910.146',
    noteKo:'미국 일반산업 허가필요 밀폐공간 프로필입니다. OSHA 1910.146은 이 도구의 10배·20 ACH를 법정값으로 정하지 않습니다. 사업장 위험평가·공학 기준을 입력하고 진입 전 및 작업 중 허용조건을 시험·감시하십시오.',
    noteEn:'US general-industry permit-space profile. OSHA 1910.146 does not make this tool’s 10-volume or 20-ACH defaults a legal requirement. Use site engineering criteria and test/monitor acceptable entry conditions.',
    source:'OSHA 29 CFR 1910.146',
    url:'https://www.osha.gov/laws-regs/regulations/standardnumber/1910/1910.146'
  },
  'us-construction':{
    label:'United States · OSHA 29 CFR 1926 Subpart AA',
    noteKo:'미국 건설업 밀폐공간 프로필입니다. 고정 환기배수 대신 현장 위험평가, 허가공간 프로그램, 격리, 시험·감시 및 구조계획을 우선 적용하십시오.',
    noteEn:'US construction confined-space profile. Apply the site hazard evaluation, permit-space program, isolation, testing/monitoring and rescue provisions; no fixed ACH is inferred here.',
    source:'OSHA 29 CFR 1926 Subpart AA',
    url:'https://www.osha.gov/laws-regs/regulations/standardnumber/1926/1926subpartaa'
  },
  uk:{
    label:'United Kingdom · Confined Spaces Regulations 1997 / HSE L101',
    noteKo:'영국 프로필입니다. 가능한 경우 밀폐공간 진입을 피하고, 불가피한 진입에는 위험성평가에 따른 안전작업체계와 작업 전 비상조치를 적용하십시오. 이 프로필은 고정 ACH를 법정값으로 제시하지 않습니다.',
    noteEn:'United Kingdom profile. Avoid confined-space entry where possible; where entry is unavoidable, use a risk-assessed safe system of work and make adequate emergency arrangements before work starts. No fixed ACH is inferred.',
    source:'UK Confined Spaces Regulations 1997 / HSE ACOP L101',
    url:'https://www.hse.gov.uk/pubns/priced/l101.pdf',
    extraSources:[['HSE confined-space legislation','https://www.hse.gov.uk/confinedspace/legislation.htm']]
  },
  au:{
    label:'Australia · WHS Regulations / Model Code of Practice',
    noteKo:'호주 모델 기준 프로필입니다. 위험성평가·진입허가·격리·대기검사·감시·비상절차를 적용하십시오. Model Code의 법적 효력과 세부 요구사항은 해당 주·준주 규제기관에서 다시 확인해야 합니다.',
    noteEn:'Australian model profile. Apply risk assessment, entry permit, isolation, atmospheric testing/monitoring and emergency procedures. Confirm the Code’s legal effect and local requirements with the relevant state or territory regulator.',
    source:'Safe Work Australia Model Code of Practice: Confined spaces',
    url:'https://www.safeworkaustralia.gov.au/sites/default/files/2024-11/model_code_of_practice-confined_spaces-nov24.pdf',
    extraSources:[['Safe Work Australia landing page','https://www.safeworkaustralia.gov.au/doc/model-code-practice-confined-spaces']]
  },
  sg:{
    label:'Singapore · WSH (Confined Spaces) Regulations 2009',
    noteKo:'싱가포르 프로필입니다. 진입허가, 안전평가자의 가스검사, 환기, 작업 중 감시·주기시험, 훈련된 감시인과 구조계획을 적용하십시오. 산소 19.5~23.5%, 가연성 10% LEL 미만 및 물질별 노출기준을 확인합니다.',
    noteEn:'Singapore profile. Apply entry permits, gas testing by a confined-space safety assessor, ventilation, monitoring/periodic testing, a trained attendant and rescue planning. Check oxygen 19.5–23.5%, flammables below 10% LEL and applicable toxic exposure limits.',
    source:'Singapore WSH (Confined Spaces) Regulations 2009',
    url:'https://sso.agc.gov.sg/SL/WSHA2006-S462-2009'
  }
};
...
const V04_LEGAL_DATA = {
  kr:{
    rows:[['O₂','18–23.5 vol%'],['CO₂','< 1.5 vol%'],['CO','< 30 ppm'],['H₂S','< 10 ppm'],['Flammable','< 10% LEL'],['Planning ventilation','Initial ≥10× volume · During work ≥20 ACH (KOSHA GUIDE H-80-2021)']],
    clauses:'산업안전보건기준에 관한 규칙 제618~625조 · KOSHA GUIDE H-80-2021'
  },
  'us-general':{
    rows:[['O₂','19.5–23.5 vol%'],['Flammable','Hazardous atmosphere when >10% LFL'],['Toxic','> applicable OSHA PEL = hazardous atmosphere'],['Test order','O₂ → flammable gases/vapors → potential toxic contaminants'],['Ventilation','No universal statutory ACH; verify acceptable entry conditions by testing and monitoring']],
    clauses:'OSHA 29 CFR 1910.146(c)–(k)'
  },
  'us-construction':{
    rows:[['O₂','19.5–23.5 vol%'],['Flammable','Hazardous atmosphere when >10% LFL'],['Toxic','> applicable OSHA PEL = hazardous atmosphere'],['Monitoring','Continuous monitoring unless employer demonstrates periodic monitoring is sufficient'],['Ventilation','No universal statutory ACH; permit-space program and verified entry conditions govern']],
    clauses:'OSHA 29 CFR 1926.1203–1211'
  },
  uk:{
    rows:[['O₂','Normal air ≈20.8 vol%; no single universal statutory entry band'],['Flammable','DSEAR risk assessment and safe system of work'],['Toxic','Applicable COSHH Workplace Exposure Limit (WEL)'],['Entry','Avoid entry where reasonably practicable'],['Emergency','Suitable and sufficient arrangements before work starts']],
    clauses:'Confined Spaces Regulations 1997, Reg. 4–5 · HSE ACOP L101'
  },
  au:{
    rows:[['O₂','19.5–23.5 vol%'],['Flammable','Target <5% LEL · 5–<10% requires calibrated continuous detector/withdrawal · ≥10% withdraw immediately'],['Toxic','< applicable Australian workplace exposure standard'],['Purge gas','Do not use pure oxygen or gas mixture >21% O₂'],['Legal effect','Confirm adoption and current requirements with the state/territory regulator']],
    clauses:'Model WHS Regulations 66–72 · Safe Work Australia Model Code, Nov 2024'
  },
  sg:{
    rows:[['O₂','19.5–23.5 vol%'],['Flammable','< 10% LEL'],['Toxic','≤ applicable permissible exposure level'],['Permit','Gas test by safety assessor and approval by authorised manager'],['Monitoring','Maintain ventilation; monitor and periodically retest during work']],
    clauses:'WSH (Confined Spaces) Regulations 2009, Reg. 4–23'
  }
};
...
function setJurisdictionProfile(value){
  ...
  if(src){
    const links=[[p.source,p.url],...(p.extraSources||[])].filter(x=>x[1]).map(([label,url])=>
      `<a href="${escapeV04(url)}" target="_blank" rel="noopener noreferrer">${escapeV04(label)}</a>`).join(' · ');
    src.innerHTML=`<b>${escapeV04(terms[1])}:</b> ${links} <span>· 2026-07-31 · ${escapeV04(terms[4])}</span>`;
  }
...
html = replace_once(
  ...
  addFanRow(currentUiLanguage==='ko'?'이동식 송풍기 #1':((PRINT_I18N[currentUiLanguage]||PRINT_I18N.en).l.equipment+' #1'), 500, 75, false);
    ...
    "fault-isolated base initialization",
)
